...
<section class="hero-section">
    ...
    <div class="container mx-auto px-4 md:px-6 relative z-10 text-center text-white">
        ...
        <div class="flex justify-center mb-12 animate-logo w-full">
            <div class="inline-flex items-center gap-2 md:gap-3 px-3 py-2 md:px-5 md:py-3 rounded-full bg-red-500/20 border border-red-400/50 backdrop-blur-md text-red-100 shadow-lg hover:bg-red-500/30 transition-all duration-300 cursor-default group max-w-full md:max-w-4xl text-left">
                ...
                <div class="flex-shrink-0 w-px h-3 md:h-4 bg-red-400/40 mx-1"></div>
        ...
        <h1 class="text-3xl md:text-6xl font-extrabold leading-tight mb-4 md:mb-6 drop-shadow-lg tracking-tight animate-logo" style="animation-delay: 0.3s;">
            Magang BPS <span class="text-transparent bg-clip-text bg-gradient-to-r block md:inline"
            style="background-image: linear-gradient(to right, #00d9ff, #005eff);">
            Kota Tegal
        </span>
        </h1>
...
<section id="alur" class="py-16 md:py-24 bg-gray-50 relative">
    <div class="container mx-auto px-4 md:px-6">
        ...
        <div class="max-w-5xl mx-auto">
            ...
            @php
                $homeStatus = isset($pendaftaran) ? strtolower($pendaftaran->status ?? '') : '';
                $isApprovedHome = in_array($homeStatus, ['approved', 'diterima'], true);
                $isRejectedHome = in_array($homeStatus, ['rejected', 'ditolak'], true);
            @endphp

            @if(($stepStatus[3] ?? '') == 'completed' && isset($pendaftaran) && $isApprovedHome)
            <div class="bg-white rounded-2xl p-6 md:p-8 border border-green-100 text-center shadow-lg relative overflow-hidden group hover:shadow-xl transition duration-300">
                ...
                <p class="text-sm md:text-base text-gray-600 mb-6 md:mb-8 max-w-lg mx-auto">
                    Pengajuan magang Anda telah disetujui. Silakan unduh surat balasan resmi dari BPS di bawah ini.
                </p>

                @if($pendaftaran->surat_balasan_pkl)
                <a href="{{ route('pendaftaran.surat_balasan.download') }}" target="_blank"
                class="inline-flex items-center bg-green-600 text-white font-bold py-2 px-6 md:py-3 md:px-8 rounded-full hover:bg-green-700 transition shadow-lg hover:shadow-green-200/50 transform hover:-translate-y-1 text-sm md:text-base">
                ...
                class="inline-flex items-center bg-blue-600 text-white font-bold py-2 px-6 md:py-3 md:px-8 rounded-full hover:bg-blue-700 transition shadow-lg hover:shadow-blue-200/50 transform hover:-translate-y-1 text-sm md:text-base">
                <i class="fas fa-redo mr-2"></i>Daftar PKL Lagi
                </a>
                </div>

            @elseif(($stepStatus[3] ?? '') == 'completed' && isset($pendaftaran) && $isRejectedHome)
<div class="bg-white rounded-2xl p-6 md:p-8 border border-red-100 text-center shadow-lg relative overflow-hidden">
    ...
    <p class="text-sm md:text-base text-gray-600">
        Pengajuan magang Anda belum dapat kami terima saat ini.
    </p>

    {{-- Alasan Penolakan --}}
    @if(!empty($pendaftaran->rejection_reason))
    <div class="mt-4 bg-red-50 border border-red-100 rounded-xl p-4 min-h-[50px] flex items-center justify-center">
    ...
    <div class="mt-6">
        <a href="{{ route('pendaftaran.create') }}"
        class="inline-flex items-center bg-blue-600 text-white font-bold py-2 px-6 md:py-3 md:px-8 rounded-full hover:bg-blue-700 transition shadow-lg hover:shadow-blue-200/50 transform hover:-translate-y-1 text-sm md:text-base">
            <i class="fas fa-redo mr-2"></i>Coba Daftar Lagi
        </a>
    </div>
            ...
            <div class="bg-white rounded-2xl p-6 md:p-8 border border-blue-100 text-center shadow-lg relative overflow-hidden">
                ...
                <p class="text-sm md:text-base text-gray-600 max-w-lg mx-auto">
                    Berkas Anda sudah kami terima dan sedang dalam antrean verifikasi. Mohon cek halaman ini secara berkala.
                </p>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 md:p-8 border border-blue-100 text-center shadow-lg transform transition hover:scale-[1.01] relative overflow-hidden">
                ...
                <p class="text-sm md:text-base text-gray-600 mb-6 md:mb-8 max-w-lg mx-auto">
                    Satu langkah lagi! Silakan lengkapi formulir pendaftaran untuk mengajukan permohonan magang.
                </p>
                <button onclick="cekProfileDanDaftar()"
                        class="inline-flex items-center bg-blue-600 text-white font-bold py-2 px-6 md:py-3 md:px-8 rounded-full hover:bg-blue-700 transition shadow-lg hover:shadow-blue-300/50 text-sm md:text-base">

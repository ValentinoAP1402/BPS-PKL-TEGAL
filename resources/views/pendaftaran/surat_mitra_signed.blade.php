        ...
        @if($pendaftaran->surat_balasan_pkl)
            <div class="space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <a href="{{ route('pendaftaran.surat_balasan.preview') }}?v={{ urlencode(optional($pendaftaran)->updated_at) }}"
                    target="_blank"
                    class="flex items-center justify-center gap-2 px-6 py-3 bg-white text-gray-700 border border-gray-200 rounded-xl font-semibold hover:bg-gray-50 hover:border-blue-300 hover:text-blue-600 transition shadow-sm group">
                        <i class="fa-regular fa-eye group-hover:scale-110 transition-transform"></i>
                        Lihat Dokumen
                    </a>

                    {{-- Unduh PDF (Download) --}}
                    <a href="{{ route('pendaftaran.surat_balasan.download') }}"
                    class="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 hover:shadow-xl hover:-translate-y-0.5 transition transform">
